...
class LiteLLMProvider(OpenAIPlatformApiProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Reuse the configured provider timeout for LiteLLM/Bedrock requests.
        timeout = self.timeout
        log.log_info(f"LiteLLM: Using configured timeout of {timeout}s")
    ...
    async def _chat_completion_impl(self, request: ChatRequest,
        ...
        try:
                ...
                if isinstance(response, str):
                    ...
                    if raw_tool_calls:
                        native_message["tool_calls"] = [
                            {
                                "id": self._field(tc, "id"),
                                "type": self._field(tc, "type", "function"),
                                "function": {
                    ...
                    reasoning_content = self._field(message, 'reasoning_content')
                    if reasoning_content:
                        native_message["reasoning_content"] = reasoning_content
                        log.log_info(f"LiteLLM: Captured reasoning_content ({len(reasoning_content)} chars)")

                    thinking_blocks = self._field(message, 'thinking_blocks')
                    if thinking_blocks:
                        native_message["thinking_blocks"] = thinking_blocks
                        log.log_info(f"LiteLLM: Captured {len(thinking_blocks)} thinking_blocks")
            ...
            return ChatResponse(
                ...
                finish_reason=finish_reason,
                native_content=native_content_for_response
            )
